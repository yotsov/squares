       ...
       (let [json-message (json/read-str text-message :key-fn keyword)
         ...
         (case (:type json-message)
           "key-press" (case (:keycode json-message)
                         38 (when-not (detected-collision? x (dec y)) (swap! ws-connections update-in [ws-id :y] dec))
                         40 (when-not (detected-collision? x (inc y)) (swap! ws-connections update-in [ws-id :y] inc))
                         37 (when-not (detected-collision? (dec x) y) (swap! ws-connections update-in [ws-id :x] dec))
                         39 (when-not (detected-collision? (inc x) y) (swap! ws-connections update-in [ws-id :x] inc)))
